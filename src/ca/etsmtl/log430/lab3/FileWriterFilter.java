...
public class FileWriterFilter extends Thread 
{
	 ...
	 * Class constructor.
	 * 
	 * @param outputFileName the name of the file, where the output of the flow goes
	...
	/**
	 * This is the method that is called when the thread is started in SystemInitialize
	 */
	public void run() 
	{
		// Declarations
			...
			while (!done) 
			{
				...
				if (!done) 
				{
					...
					if (integerCharacter1 == -1) // pipe #1 is closed
					{
						...
						{
							System.out.println("FileWriterFilter:: Error closing input pipe.");
						} // try/catch
					}
					else 
					{
						...
						{
							System.out.println("FileWriterFilter:: Received: " + lineOfText1 + " on input pipe.");
							write1 = true;
						} 
						else 
						{
